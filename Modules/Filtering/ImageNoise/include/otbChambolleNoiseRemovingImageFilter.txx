...
#ifndef __otbChambolleNoiseRemovingImageFilter_txx
#define __otbChambolleNoiseRemovingImageFilter_txx

#include "otbChambolleNoiseRemovingImageFilter.h"
...
ChambolleNoiseRemovingImageFilter<TInputVectorImage>::ChambolleNoiseRemovingImageFilter()
{
	SetLambda(1.0);
	m_Radius.Fill(1);
...
template <class TInputVectorImage>
void
ChambolleNoiseRemovingImageFilter<TInputVectorImage>::PrintSelf(std::ostream& os, itk::Indent indent) const
{
...
template <class TInputVectorImage>
void ChambolleNoiseRemovingImageFilter<TInputVectorImage>::GenerateInputRequestedRegion() throw (itk::InvalidRequestedRegionError)
{
...
template<class TInputVectorImage>
void ChambolleNoiseRemovingImageFilter<TInputVectorImage>::BeforeThreadedGenerateData()
{
	VectorConstPointerType input  = this->GetInput();

/*std::cout << "input->GetRequestedRegion() index " << input->GetRequestedRegion().GetIndex()[0] << " " << input->GetRequestedRegion().GetIndex()[1] << std::endl;
std::cout << "input->GetRequestedRegion() size " << input->GetRequestedRegion().GetSize()[0] << " " << input->GetRequestedRegion().GetSize()[1] << std::endl;*/
...
template<class TInputVectorImage>
void ChambolleNoiseRemovingImageFilter<TInputVectorImage>::ThreadedGenerateData(const ImageRegionType& RegionForThread, int threadId)
  {
	...
	// After divergence call, m_Lambda*m_Temp3 is equivalent to the noise to be removed from the input
	divergence(m_temp3,RegionForThread);

	// Noise soustraction; the result is set to output
	...
	while(!itInput.IsAtEnd())
	{
		for (unsigned int b=0; b<input->GetNumberOfComponentsPerPixel(); b++)
			itOutput.Get()[b] = itInput.Get()[b]-m_Lambda*it3t.Get()[b];

		++itInput; ++itOutput; ++it3t;
	}
...
template<class TInputVectorImage>
void ChambolleNoiseRemovingImageFilter<TInputVectorImage>::divergence(
